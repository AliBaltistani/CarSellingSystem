                            ...
                            <div class="relative group rounded-lg overflow-hidden border border-slate-200">
                                ...
                                <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-lg flex items-center justify-center gap-2">
                                    ...
                                    <button type="button" x-show="!img.is_primary" @click="makePrimary(img, idx)"
                                        ...
                                        class="p-1 bg-white rounded text-amber-600 hover:bg-amber-100 disabled:opacity-50" title="Set Primary">
                                    ...
                                    <button type="button" @click="removeImage(img, idx)"
                                        :disabled="img.loading"
                                        class="p-1 bg-white rounded text-red-600 hover:bg-red-100 disabled:opacity-50" title="Delete">
        ...
        function adminCarEditStepper() {
            return {
                ...
                async removeImage(img, idx) {
                    ...
                    img.loading = true;
                    try {
                        const res = await fetch(`/admin/cars/images/${img.id}`, {
                            method: 'DELETE',
                ...
                async makePrimary(img, idx) {
                    img.loading = true;
                    try {
                        const res = await fetch(`/admin/cars/images/${img.id}/primary`, {
                            method: 'POST',
                            headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' }
                        });
                        if (res.ok) {
