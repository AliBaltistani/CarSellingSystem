<x-layouts.admin title="Create Banner">
    <div class="mb-6">
    ...
    <div class="bg-white rounded-xl shadow-sm">
        <div class="p-6 border-b border-slate-100">
            <h1 class="text-xl font-bold text-slate-900">Create New Banner</h1>
            <p class="text-slate-600">Add a new banner to display on the homepage slider</p>
        </div>
        ...
        <form action="{{ route('admin.banners.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            ...
            <div>
                ...
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-xl hover:border-amber-400 transition-colors">
                    <div class="space-y-1 text-center">
                        ...
                        <p class="text-xs text-slate-500">PNG, JPG, WEBP up to 5MB. Recommended: 1920x600px</p>
                    </div>
                </div>
                @error('image')
            ...
            <div>
                ...
                <textarea name="subtitle" id="subtitle" rows="2"
                    class="w-full px-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                    placeholder="Additional text to display on the banner">{{ old('subtitle') }}</textarea>
                @error('subtitle')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
