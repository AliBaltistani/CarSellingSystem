    ...
    <div class="max-w-2xl mx-auto">
        ...
        <form action="{{ $group->exists ? route('admin.attribute-groups.update', $group) : route('admin.attribute-groups.store') }}" 
            ...
            <div class="space-y-6">
                ...
                <!-- Icon -->
                <div>
                    <label for="icon" class="block text-sm font-medium text-slate-700 mb-2">Icon (SVG markup)</label>
                    <div class="flex gap-3 items-start">
                        <div class="flex-1">
                            <textarea id="icon" name="icon" rows="3"
                                class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent font-mono text-xs"
                                placeholder='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">...</svg>'
                                oninput="document.getElementById('icon-preview').innerHTML = this.value">{{ old('icon', $group->icon) }}</textarea>
                        </div>
                        <div id="icon-preview" class="w-12 h-12 flex items-center justify-center bg-slate-100 rounded-lg border border-slate-200 text-slate-600 shrink-0">
                            {!! old('icon', $group->icon) !!}
                        </div>
                    </div>
                    <p class="mt-1 text-xs text-slate-500">Paste an inline SVG icon. Use Heroicons outline style for consistency.</p>
