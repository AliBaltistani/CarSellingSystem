<x-layouts.public>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="carFormStepper()">
        ...
        <form action="{{ route('cars.update', $car) }}" method="POST" enctype="multipart/form-data" id="car-form">
            ...
            <div x-show="currentStep === 5" data-step="5" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
                ...
                <!-- Current Images -->
                <div x-show="existingImages.length > 0" class="bg-white rounded-xl shadow-sm p-6 mb-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <template x-for="(img, idx) in existingImages" :key="img.id">
                            <div class="relative group rounded-lg overflow-hidden border border-slate-200">
                                ...
                                <!-- Primary badge -->
                                <span x-show="img.is_primary" class="absolute top-1 left-1 px-2 py-0.5 bg-amber-500 text-white text-xs rounded font-medium">Primary</span>
                                <!-- Hover overlay with actions -->
                                <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                    <!-- Set Primary -->
                                    <button type="button" x-show="!img.is_primary" @click="makePrimary(img, idx)"
                                        :disabled="img.loading"
                                        class="p-1.5 bg-white rounded-lg text-amber-600 hover:bg-amber-50 transition-colors disabled:opacity-50" title="Set as Primary">
                                        <svg x-show="!img.loading" class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                        <svg x-show="img.loading" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                                    </button>
                                    <!-- Delete -->
                                    <button type="button" @click="removeImage(img, idx)"
                                        :disabled="img.loading"
                                        class="p-1.5 bg-white rounded-lg text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50" title="Delete Image">
                                        <svg x-show="!img.loading" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                        <svg x-show="img.loading" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                                    </button>
                                </div>
                            </div>
    ...
    <script>
        ...
        @php
            $existingImagesData = $car->images->map(fn($img) => ['id' => $img->id, 'url' => $img->url, 'is_primary' => $img->is_primary, 'loading' => false]);
        @endphp
        function carFormStepper() {
            return {
                ...
                async removeImage(img, idx) {
                    if (!confirm('Are you sure you want to delete this image?')) return;
                    img.loading = true;
                    try {
                        const res = await fetch(`/my-cars/images/${img.id}`, {
                            method: 'DELETE',
                        ...
                        if (res.ok) {
                            ...
                            if (img.is_primary && this.existingImages.length > 0) {
                ...
                async makePrimary(img, idx) {
                    img.loading = true;
                    try {
                        const res = await fetch(`/my-cars/images/${img.id}/primary`, {
                            method: 'POST',
                            headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' }
                        });
                        if (res.ok) {
                            this.existingImages.forEach(i => i.is_primary = false);
                            img.is_primary = true;
                        } else {
